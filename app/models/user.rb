class User < ApplicationRecord
  ...
  def segmented_leaves(date_used, company, leave_type)
    ...
    employment.each do |emp|
      value += emp
               ...
               .where(leave_request_id: leave_requests.ids)
  ...
  def total_leaves(date_used, company)
    leave_amounts.where(date: date_used).where('employments.company_id = ?', company.id).sum(:amount)
  end
